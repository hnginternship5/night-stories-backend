...
router.post(
  ...
  (req, res) => {
    ...
    // }
    //A new story is been created here
    ...
    User.findById( req.user.id).then(user => {
      ...
      Story.findById(req.params.id)
        .then(story => {
          ...
          // Save the story instance
          story.save().then(story => res.json({liked:true,
      ...
      Story.findById(req.params.id)
        .then(story => {
          ...
          if (
            ...
            return res
              .status(400)
              .json({ alreadyliked: 'User already liked this Story',
                      liked:true});
          ...
          story.likes.unshift({ user: req.user.id  });
          // Save the story instance
          story.save().then(story => res.json({story:story, user:user,justLiked:'you just like the Story'}));
    ...
    User.findById( req.user.id ).then(user => {
      ...
      Story.findById(req.params.id)
        .then(story => {
          ...
          story.likes.unshift({ user: req.user.id  });
          //Save the User instance
          story.save().then(post => res.json({post:post,liked:true, status:'you just like this story'}));
        })
        .catch(err => {res.status(404).json({ postnotfound: 'No post found' })
        ...
        Story.findById(req.params.id)
          .then(story => {
            ...
            if (
              ...
              return res
                .status(400)
                .json({ status: 'You have not yet liked this post',
                          liked:false });
            }
            //oh it exist
            // Get remove index
